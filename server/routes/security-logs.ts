...
export const handleSecurityHealth: RequestHandler = (req, res) => {
  try {
    ...
    if (health.metrics.malwareDetected > 0) {
      ...
      health.issues.push(`${health.metrics.malwareDetected} malware detection(s) in last 24h`);
    }
    
    if (health.metrics.quarantinedFiles > 10) {
      health.status = health.status === 'critical' ? 'critical' : 'warning';
      health.issues.push(`High quarantine rate: ${health.metrics.quarantinedFiles} files`);
    }
    
    if (health.metrics.criticalEvents > 5) {
      health.status = health.status === 'critical' ? 'critical' : 'warning';
      health.issues.push(`${health.metrics.criticalEvents} critical security events`);
    }
...
export const handleSecurityReport: RequestHandler = (req, res) => {
  try {
    ...
    if (format === 'csv') {
      ...
      const csvHeader = 'Timestamp,Level,Event Type,Message,Severity,User ID,File Name,IP\n';
