...
export class SecurityLogger {
  ...
  private getRecentEventCount(eventType: SecurityEventType, minutes: number): number {
    try {
      ...
      for (const line of lines) {
        try {
          ...
          if (entry.eventType === eventType && new Date(entry.timestamp) > cutoffTime) {
            count++;
          }
        } catch {
  ...
  private triggerAlert(eventType: SecurityEventType, count: number, threshold: number): void {
    ...
    console.error(`🚨 SECURITY ALERT: ${count} ${eventType} events in the last hour (threshold: ${threshold})`);
  }
  ...
  logFileUpload(userId: string, fileName: string, fileSize: number, ip?: string, userAgent?: string): void {
    this.log(
  ...
  logFileValidation(fileName: string, fileHash: string, isValid: boolean, reasons?: string[]): void {
    this.log(
  ...
  logFileQuarantine(fileName: string, fileHash: string, reasons: string[], userId?: string): void {
    this.log(
  ...
  logMalwareDetected(fileName: string, fileHash: string, malwareType: string, userId?: string): void {
    this.log(
  ...
  logSuspiciousActivity(activity: string, details: Record<string, any>, userId?: string, ip?: string): void {
    this.log(
  ...
  logAccessAttempt(resource: string, success: boolean, userId?: string, ip?: string, userAgent?: string): void {
    this.log(
  ...
  getLogStats(days: number = 7): {
    ...
    const stats = {
      ...
      topThreats: [] as Array<{ fileName: string; count: number; reasons: string[] }>
    };
