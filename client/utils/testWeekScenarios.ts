 ...
 */
import { getWeekForDate, getAllWeeks, formatDateBR, formatDateFullBR } from './weekSystem';
...
export function runWeekTests() {
  ...
  const week1Exists = allWeeks.find(w => w.week === week1.week && w.year === week1.year);
  const week2Exists = allWeeks.find(w => w.week === week2.week && w.year === week2.year);

  console.log("🔍 VERIFICAÇÃO DE EXISTÊNCIA NO SISTEMA");
  console.log(`   Semana ${week1.week} de ${week1.year} existe: ${week1Exists ? '✅ SIM' : '❌ NÃO'}`);
  if (week1Exists) {
    console.log(`   Período: ${week1Exists.startDate} - ${week1Exists.endDate}`);
  }
  
  console.log(`   Semana ${week2.week} de ${week2.year} existe: ${week2Exists ? '✅ SIM' : '❌ NÃO'}`);
  if (week2Exists) {
    console.log(`   Período: ${week2Exists.startDate} - ${week2Exists.endDate}`);
  }
  console.log("");
  ...
  const currentWeekExists = allWeeks.find(w => w.week === currentWeek.week && w.year === currentWeek.year);
  
  console.log(`   Data atual: ${now.toLocaleDateString("pt-BR")}`);
  console.log(`   Semana atual: ${currentWeek.week} de ${currentWeek.year}`);
  console.log(`   Semana atual existe no sistema: ${currentWeekExists ? '✅ SIM' : '❌ NÃO'}`);
  
  if (currentWeekExists) {
    console.log(`   Período atual: ${currentWeekExists.startDate} - ${currentWeekExists.endDate}`);
  }
