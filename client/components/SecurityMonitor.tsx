...
export default function SecurityMonitor() {
  ...
  const loadSecurityData = async () => {
    try {
      ...
      // Load recent alerts
      const alertsResponse = await fetch('/api/security/alerts?days=1&minSeverity=6');
  ...
  const downloadReport = async (format: 'json' | 'csv' = 'json') => {
    try {
      const response = await fetch(`/api/security/report?days=7&format=${format}`);
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center p-8">
        ...
        <span className="ml-2 text-gray-600">Carregando monitor de segurança...</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-gray-900">🔍 Monitor de Segurança</h2>
          <p className="text-gray-600">Monitoramento em tempo real do sistema de upload</p>
        </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-lg">
                  Status do Sistema: {health.status === 'healthy' ? 'Saudável' : health.status === 'warning' ? 'Atenção' : 'Crítico'}
                </CardTitle>
              </div>
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
              <div className="text-center">
                <div className="text-2xl font-bold text-blue-600">{health.metrics.totalEvents}</div>
                <div className="text-xs text-gray-500">Eventos Totais</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-red-600">{health.metrics.criticalEvents}</div>
                <div className="text-xs text-gray-500">Eventos Críticos</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-orange-600">{health.metrics.quarantinedFiles}</div>
                <div className="text-xs text-gray-500">Arquivos em Quarentena</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-red-600">{health.metrics.malwareDetected}</div>
              ...
              <div className="space-y-2">
                ...
                <ul className="space-y-1">
                  {health.issues.map((issue, index) => (
                    <li key={index} className="text-sm bg-white/50 px-2 py-1 rounded border">
                      {issue}
                    </li>
                  ))}
                </ul>
      ...
      <div className="text-center text-xs text-gray-500">
        <div className="flex items-center justify-center gap-2">
          ...
          {autoRefresh ? 'Monitoramento ativo - Atualização automática a cada 30s' : 'Monitoramento pausado'}
        </div>
      </div>
    </div>
  );
